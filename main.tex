...
坂道の周辺では、植え込みで自己位置推定していた。
しかし、マップを取った時と本走行の時の植え込みの状態が変化していたために、
自己位置が正しく推定できずに破綻してしまった。
...
たまチームは、２次元の LiDAR と IMU を、
Raspberry Pi 4 を通してロボットに搭載したノート PC に送信し、
ノート PC でナビゲーションを実行し、
制御司令を Raspberry Pi 4 に返すシステムになっている。 % 画像貼りたい %
自己位置推定については、昨年度まで使用されていた emcl2 パッケージが
ROS 2 に移行されたのでそれを利用した。

\subsubsection{実験走行、本走行で見つかった課題}

\paragraph{ロボットの旋回による自己位置の破綻}
たまチームはつくばチャレンジ EX で屋内での自律走行をしていた。
その際は IMU を搭載しておらず、
ナビゲーション時にロボットが人混みを避けようとして頻繁に旋回し、
自己位置が破綻した。
そこで、旋回時の自己位置破綻対策として IMU を実装し、
車輪の速度差から推定していたヨー軸の角度を IMU から得られるヨー方向の角速度の積分値に置き換えた。
IMU を搭載した効果はマッピングの際に確認できた。
マップの取得は人がコントローラを用いてロボットを操作する。
IMU を搭載していなかったときはこの際に、
操作ミスやタイヤのスリップによる旋回をすることで、
ひどく歪んだマップが生成されてしまうことがあった。
そこに IMU が搭載されたことでマップの歪みが減少した。
ナビゲーション中においても突然旋回することが何度もあったが、
その旋回が自己位置推定の破綻に繋がったことは無かった。
